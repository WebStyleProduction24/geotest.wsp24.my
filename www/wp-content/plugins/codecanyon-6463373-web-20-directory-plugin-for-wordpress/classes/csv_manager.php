	...
	public function processDirectoryLocations(&$listing_data, $line_n) {
		foreach ($listing_data['locations'] as $location_item) {
			if (!is_numeric($location_item)) {
					...
					if ($term = term_exists(htmlspecialchars($location_name), W2DC_LOCATIONS_TAX, $listing_term_id)) { // htmlspecialchars() needed due to &amp; symbols in import files
						...
						if ($this->if_term_not_found == 'create') {
							...
							if (!is_wp_error($newterm)) {
								...
								$error = sprintf(__('line %d: ', 'W2DC') . __('Something went wrong with directory location "%s"', 'W2DC'), $line_n, $location_name);
								$this->log['errors'][] = $error;
								echo $error;
								echo "<br />";
							}
						} else {
							$error = sprintf(__('line %d: ', 'W2DC') . esc_attr__("Directory location \"%s\" wasn't found, was skipped", 'W2DC'), $line_n, $location_name);
							$this->log['errors'][] = $error;
							echo $error;
							echo "<br />";
						}
				...
				$error = sprintf(__('line %d: ', 'W2DC') . esc_attr__("Directory location with ID \"%d\" wasn't found", 'W2DC'), $line_n, $location_item);
				$this->log['errors'][] = $error;
				echo $error;
				echo "<br />";
			}
				...
				if ($term = term_exists(htmlspecialchars($category_name), W2DC_CATEGORIES_TAX, $listing_term_id)) { // htmlspecialchars() needed due to &amp; symbols in import files
					...
					if ($this->if_term_not_found == 'create') {
						...
						if (!is_wp_error($newterm)) {
							...
							$error = sprintf(__('line %d: ', 'W2DC') . __('Something went wrong with directory category "%s"', 'W2DC'), $line_n, $category_name);
							$this->log['errors'][] = $error;
							echo $error;
							echo "<br />";
						}
					} else {
						$error = sprintf(__('line %d: ', 'W2DC') . esc_attr__("Directory category \"%s\" wasn't found, was skipped", 'W2DC'), $line_n, $category_name);
						$this->log['errors'][] = $error;
						echo $error;
						echo "<br />";
					}
			...
			if ($term = term_exists(htmlspecialchars($tag_name), W2DC_TAGS_TAX)) { // htmlspecialchars() needed due to &amp; symbols in import files
				...
				if ($this->if_term_not_found == 'create') {
					...
					if (!is_wp_error($newterm))
						...
						$error = sprintf(__('line %d: ', 'W2DC') . __('Something went wrong with directory tag "%s"', 'W2DC'), $line_n, $tag_name);
						$this->log['errors'][] = $error;
						echo $error;
						echo "<br />";
					}
				} else {
					$error = sprintf(__('line %d: ', 'W2DC') . esc_attr__("Directory tag \"%s\" wasn't found, was skipped", 'W2DC'), $line_n, $tag_name);
					$this->log['errors'][] = $error;
					echo $error;
					echo "<br />";
				}
		...
		foreach ($listing_data['videos'] AS $video_item) {
			...
			if ($validation->valid_url($video_item, false)) {
				...
				elseif (isset($matches_vimeo[5]) && strlen($matches_vimeo[5]) == 9) {
					...
					$error = sprintf(__('line %d: ', 'W2DC') . esc_attr__("YouTube or Vimeo video URL is incorrect", 'W2DC'), $line_n);
					$this->log['errors'][] = $error;
					echo $error;
					echo "<br />";
				}
