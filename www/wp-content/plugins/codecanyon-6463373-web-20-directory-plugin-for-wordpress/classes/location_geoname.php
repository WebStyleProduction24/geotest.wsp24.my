...
class w2dc_locationGeoname {
	...
	public function geocodeRequest($query, $return = 'geoname') {
		...
		if (get_option('w2dc_google_api_key'))
			$fullUrl = sprintf("https://maps.googleapis.com/maps/api/place/textsearch/json?query=%s&language=en&key=%s%s", urlencode($query), get_option('w2dc_google_api_key'), $region);
		else
