...
class w2dc_categories_manager {
	...
	public function taxonomy_rows($row, $column_name, $term_id) {
		...
		if ($column_name == 'w2dc_category_image') {
			...
			return $row . '<a href="' . get_edit_term_link($term_id, W2DC_CATEGORIES_TAX, W2DC_POST_TYPE) . '">' . ($url ? '<img src="' . $url . '" width="100">' : '') . __("Select image", "W2DC") . '</a>';
		}
	...
	public function select_category_marker_png_image() {
		if (isset($_POST['category_id']) && is_numeric($_POST['category_id'])) {
			$category_id = $_POST['category_id'];
			$markers_images = get_option('w2dc_categories_marker_images');
	...
	public function select_category_marker_icon() {
		if (isset($_POST['category_id']) && is_numeric($_POST['category_id'])) {
			$category_id = $_POST['category_id'];
			$markers_icons = get_option('w2dc_categories_marker_icons');
	...
	public function select_category_marker_color() {
		if (isset($_POST['category_id']) && is_numeric($_POST['category_id'])) {
			$category_id = $_POST['category_id'];
			$markers_colors = get_option('w2dc_categories_marker_colors');
	...
	public function save_image($term_id) {
		...
		$categories_images = get_option('w2dc_categories_images');
