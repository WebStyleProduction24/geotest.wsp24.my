...
function w2dc_tax_dropdowns_menu_init($params) {
	...
	if ($terms) {
		...
		echo '<div class="w2dc-row w2dc-form-group">';
		...
		foreach ($terms AS $term) {
			...
			echo '<option id="' . $term->slug . '" value="' . $term->term_id . '" data-name="' . $term->name  . '" data-parents="" ' . $selected . ' ' . $icon . '>' . $term->name . $term_count . '</option>';
			if ($depth > 1) {
...
function _w2dc_tax_dropdowns_menu($tax, $parent = 0, $depth = 2, $current_level = 1, $term_id = null, $count = false, $exact_terms = array(), $hide_empty = false) {
	...
	if ($terms && ($depth == 0 || !is_numeric($depth) || $depth > $current_level)) {
		...
		if ($terms) {
			...
			$parents = w2dc_get_term_parents($term->parent, $tax, false, false, ', ');
			...
			foreach ($terms AS $term) {
				...
				echo '<option id="' . $term->slug . '" value="' . $term->term_id . '" data-name="' . $term->name  . '" data-parents="' . $parents . '" ' . $selected . ' ' . $icon . '>' . $term->name . $term_count . '</option>';
